...
@HiltViewModel
class LoginViewModel @Inject constructor(var authRepo: AuthRepository) : ViewModel() {
    var user = MutableLiveData<MyUser>()
    var isLoading = authRepo.isLoading

    init {
        Log.e("viewc", " Login View Model user :${user.value} init çalıştı")
    }
    ...
    fun login(email: String, password: String, context: Context) {
        CoroutineScope(Dispatchers.Main).launch {
            withContext(coroutineContext) {
                user.value = authRepo.login(email, password, context).value
            }
        }
    }
}